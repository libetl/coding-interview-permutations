...
const permutations = (arraysOfLetters, length, prefix = []) => !length ? [prefix.join('')] : arraysOfLetters.map(
    arrayOfLetter => arrayOfLetter.map(elem => permutations( arraysOfLetters.withoutArrayContaining(elem),  length - 1,
        [...prefix, elem]).flatMap()).flatMap()).flatMap()

export default number => permutations(rangeOfLetters(number), Math.max(0, Math.floor(Math.log10(number)) + 1)).filter(result => result)